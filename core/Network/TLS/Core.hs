...
recvData1 ctx = do
  ...
  where process (Handshake [ch@ClientHello{}]) =
        ...
        process (Alert [(AlertLevel_Fatal, desc)]) = do
            setEOF ctx
            E.throwIO (Terminated True ("received fatal error: " ++ show desc) (Error_Protocol ("remote side fatal error", AlertLevel_Fatal, desc)))
  ...
  where process (Alert13 [(AlertLevel_Warning, UserCanceled)]) = return B.empty
        ...
        process (AppData13 x) = do
            ...
            case established of
              ...
              NotEstablished      -> throwCore $ Error_Protocol ("data at not-established", AlertLevel_Fatal, UnexpectedMessage)
...
keyUpdate ctx getState setState = do
    ...
    unless (level == CryptApplicationSecret) $
        throwCore $ Error_Protocol ("tried key update without application traffic secret", AlertLevel_Fatal, InternalError)

...
onServerHello ctx cparams clientSession sentExts (ServerHello rver serverRan serverSession cipher compression exts) = do
    when (rver == SSL2) $
        throwCore $
            Error_Protocol "SSL2 is not supported" ProtocolVersion
    when (rver == SSL3) $
        throwCore $
            Error_Protocol "SSL3 is not supported" ProtocolVersion
    ...
    if ver > TLS12
        then do
            when (serverSession /= clientSession) $
                throwCore $
                    Error_Protocol "received mismatched legacy session" IllegalParameter

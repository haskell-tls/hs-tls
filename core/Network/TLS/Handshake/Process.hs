...
processHandshake ctx hs = do
  ...
  where secureRenegotiation = supportedSecureRenegotiation $ ctxSupported ctx
        ...
        processClientExtension (ExtensionRaw 0xff01 content) | secureRenegotiation = do
            ...
            unless (bs `bytesEq` content) $ throwError $ Error_Protocol ("client verified data not matching: " ++ show v ++ ":" ++ show content, AlertLevel_Fatal, HandshakeFailure)
        ...
        processCertificates ClientRole (CertificateChain []) =
            throwCore $ Error_Protocol ("server certificate missing", AlertLevel_Fatal, HandshakeFailure)
...
processClientKeyXchg ctx (CKX_DH clientDHValue) = do
    ...
    unless (dhValid params $ dhUnwrapPublic clientDHValue) $
        throwCore $ Error_Protocol ("invalid client public key", AlertLevel_Fatal, IllegalParameter)
...
processClientKeyXchg ctx (CKX_ECDH bytes) = do
    ...
    case decodeGroupPublic grp bytes of
      Left _ -> throwCore $ Error_Protocol ("client public key cannot be decoded", AlertLevel_Fatal, IllegalParameter)
      Right clipub -> do
          ...
          case groupGetShared clipub srvpri of
              Just premaster -> do
                  ...
                  masterSecret <- usingHState ctx $ setMasterSecretFromPre rver role premaster
                  liftIO $ logKey ctx (MasterSecret masterSecret)
              Nothing -> throwCore $ Error_Protocol ("cannot generate a shared secret on ECDH", AlertLevel_Fatal, IllegalParameter)

...
checkFinished ctx usedHash baseKey hashValue verifyData = do
    ...
    when (B.length verifyData /= B.length verifyData') $ throwCore $ Error_Protocol ("broken Finished", AlertLevel_Fatal, DecodeError)
...
makeServerKeyShare ctx (KeyShareEntry grp wcpub) = case ecpub of
  Left  e    -> throwCore $ Error_Protocol (show e, AlertLevel_Fatal, IllegalParameter)
  Right cpub -> do
      ecdhePair <- generateECDHEShared ctx cpub
      case ecdhePair of
          Nothing -> throwCore $ Error_Protocol (msgInvalidPublic, AlertLevel_Fatal, IllegalParameter)
          Just (spub, share) ->
...
fromServerKeyShare (KeyShareEntry grp wspub) cpri = case espub of
  ...
  Right spub -> case IES.groupGetShared spub cpri of
    ...
    Nothing     -> throwCore $ Error_Protocol ("cannot generate a shared secret on (EC)DH", AlertLevel_Fatal, IllegalParameter)
  where
...
ensureNullCompression compression =
    when (compression /= compressionID nullCompression) $
        throwCore $ Error_Protocol ("compression is not allowed in TLS 1.3", AlertLevel_Fatal, IllegalParameter)

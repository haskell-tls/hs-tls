...
runTLSPipeN
    ...
    -> (Context -> Chan [C8.ByteString] -> IO ())
    -> (Chan C8.ByteString -> Context -> IO ())
    -> PropertyM IO ()
...
runTLSPipePredicate
    :: (ClientParams, ServerParams) -> (Maybe Information -> Bool) -> PropertyM IO ()
...
runTLSPipeCapture13
    :: (ClientParams, ServerParams) -> PropertyM IO ([Handshake13], [Handshake13])
...
assertEq expected got =
    unless (expected == got) $
        error ("got " ++ show got ++ " but was expecting " ++ show expected)
  ...
  where
    ...
    tests_handshake =
        testGroup
            ...
            , testProperty
                "Certificate fallback (ciphers)"
                (monadicIO prop_handshake_cert_fallback)
            , testProperty
                "Certificate fallback (hash and signatures)"
                (monadicIO prop_handshake_cert_fallback_hs)
            ...
            , testProperty
                "Extended Master Secret (resumption)"
                (monadicIO prop_handshake_session_resumption_ems)

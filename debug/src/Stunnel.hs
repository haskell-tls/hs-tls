...
connectAddressDescription (AddrSocket family sockaddr) = do
    ...
    E.catch
        ...
        ( \(SomeException e) ->
            close sock >> error ("cannot open socket " ++ show sockaddr ++ " " ++ show e)
        )
...
listenAddressDescription (AddrSocket family sockaddr) = do
    sock <- socket family Stream defaultProtocol
    E.catch
        (bind sock sockaddr >> listen sock 10 >> setSocketOption sock ReuseAddr 1)
        ( \(SomeException e) ->
            close sock >> error ("cannot open socket " ++ show sockaddr ++ " " ++ show e)
        )
    return $ StunnelSocket sock
...
doServer source destination flags = do
    creds <-
        (either (error . show) Credentials . sequence)
            `fmap` mapM loadCred (zip (getCertificate flags) (getKey flags))
    srcaddr <- getAddressDescription source
    dstaddr <- getAddressDescription destination
...
options =
    [ Option
        ['s']
        ["source"]
        (ReqArg Source "source")
        "source address influenced by source type"
    , Option
        ['d']
        ["destination"]
        (ReqArg Destination "destination")
        "destination address influenced by destination type"
    , Option
        []
        ["source-type"]
        (ReqArg SourceType "source-type")
        "type of source (tcp, unix, fd)"
    , Option
        []
        ["destination-type"]
        (ReqArg DestinationType "source-type")
        "type of source (tcp, unix, fd)"
    , Option
        []
        ["debug"]
        (NoArg Debug)
        "debug the TLS protocol printing debugging to stdout"
